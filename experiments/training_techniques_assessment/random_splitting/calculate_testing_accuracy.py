...
from configurations import INF, MODELS
...
def main():
    for model in MODELS:
        validation_predictions_address = f"results/{model}/prediction/validation_predictions.csv"
        ...
        testing_predictions_address = f"results/{model}/prediction/testing_predictions.csv"
        ...
        testing_accuracy_df = pd.DataFrame(data={"accuracy": [testing_accuracy],
                                                 ...
                                                 "optimal threshold": [optimal_threshold]})
        validation_accuracy_df.to_csv(f"results/{model}/accuracy/validation_accuracy.csv", index=False)
        testing_accuracy_df.to_csv(f"results/{model}/accuracy/testing_accuracy.csv", index=False)
    return
