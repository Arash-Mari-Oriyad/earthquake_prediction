...
from configurations import HISTORICAL_DATA_BASE_ADDRESS, FORECAST_HORIZON, HISTORY_LENGTHS, \
    MODELS, RESULTS_BASE_ADDRESS, VALIDATION_SIZE, TESTING_SIZE, GROUPS


def main():
    for group in GROUPS:
        input_data_addresses = \
            [f"{HISTORICAL_DATA_BASE_ADDRESS}/{'_'.join([str(sub_region_id) for sub_region_id in group])}" \
